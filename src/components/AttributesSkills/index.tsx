...
const AttsAndSkills = () => {
  ...
  const [dexValue, setDexValue] = useState<number>(0);
  const [dexModifier, setDexModifier] = useState<number>(0);

  const [percValue, setPercValue] = useState<number>(0);
  const [percModifier, setPercModifier] = useState<number>(0);

  const [strValue, setStrValue] = useState<number>(0);
  const [strModifier, setStrModifier] = useState<number>(0);

  const [knowValue, setKnowValue] = useState<number>(0);
  const [knowModifier, setKnowModifier] = useState<number>(0);

  const [techValue, setTechValue] = useState<number>(0);
  const [techModifier, setTechModifier] = useState<number>(0);

  const [mechValue, setMechValue] = useState<number>(0);
